...
def parse(courses):
    ...
    for crsln in courses:
        match = re.match(r'([A-Z] [A-Z][A-Z]|[A-Z][A-Z] [A-Z]|[A-Z] [A-Z]|[A-Z]{2,4})\s*(\d{3,4}[A-Z]?)(?:\s*-\s*|\s+)(\d{1,2})(?:\s+|:\s*)(.*), (\w+) (\d+)', crsln, re.I)
        if match:
            ...
            print "no match"
    if len(ctree) > 1:
        res = ' / '.join(dept + ' ' + cnum + ' - ' + ', '.join(sorted(ctree[dept][cnum]))
                         for dept in sorted(ctree) for cnum in sorted(ctree[dept]))
    else:
        res = ' / '.join(dept + ' ' + ' / '.join(cnum + ' - ' + ', '.join(sorted(ctree[dept][cnum]))
